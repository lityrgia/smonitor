...
static NTSTATUS WideToAscii(const wchar_t* src, char* dest, size_t destSize)
{
...
void* FindModuleBase(const wchar_t* moduleName, ULONG* size)
{
	...
	WideToAscii(moduleName, moduleNameAscii, sizeof(moduleNameAscii));

	auto info = reinterpret_cast<SYSTEM_MODULE_INFORMATION*>(ExAllocatePool2(
		POOL_FLAG_NON_PAGED, needSize, POOL_TAG));
